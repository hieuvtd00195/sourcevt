...
const ProFormSelectSorted = <O extends FieldValues, V extends string | number>(
  props: Props<O, V>
) => {
  const {
    name,
  ...
  const sortedEntries: any = Object.values(entries).sort((a, b) => a.label.localeCompare(b.label));
  ...
  return (
    <FormControl fullWidth error={Boolean(error)} disabled={disabled}>
      ...
      >
        ...
        {Object.keys(sortedEntries).map((valueKey) => {
          const { value, label, disabled, key } = sortedEntries[valueKey];
          return (
...
export default ProFormSelectSorted;

...
const schema = Validation.shape(
  {
    storeId: Validation.string()
      ...
      .default(null),
    audienceType: Validation.option()
        ...
        yup.array().of(
          yup.object().shape({
            ...
            quantity: Validation.pattern(
              ...
              .required()
              .when('$billType', (billType: any, schema: any) => {
                if (Number(billType) === 1) {
                  return schema.test({
                    name: 'quantity',
                    message: 'Không được lớn hơn số lượng tồn',
                    test: (quantity: any, context: any) => {
                      const { productStockQuantity } = context.parent;
                      console.log(context);
                      if (Number(quantity) > productStockQuantity) {
                        return false;
                      }
                      return true;
                    },
    ...
    documentDetailType: Validation.option().required(
      'Kiểu không được để trống'
    ),
    note: Validation.string().optional(),
  },
  [
...
const UpdateBill = () => {
  ...
  const handleSubmit = async (data: any) => {
    ...
    // const products = form.mforEachp((item: TableCreateProducts) => ({
    //   productId: item.productId,
    //   //   unit: item.unit ? Number(item.unit) : 0,
    //   quantity: Number(item.quantity),
    //   price: Number(item.price),
    ...
    const body = {
      ...
      cashPaymentAmount:
        cashPaymentAmount && Number(cashPaymentAmount) !== 0
          ? cashPaymentAmount
          : null,
  ...
  return (
    <PageWrapper
      title={billType === '0' ? 'Sửa phiếu nhập' : 'Sửa phiếu xuất' || ''}
    >
      <PageBreadcrumbs
        title={billType === '0' ? 'Sửa phiếu nhập' : 'Sửa phiếu xuất' || ''}
        items={[
                            ...
                            <Grid item xs={2} sm={2} md={2} lg={2}>
                              <ProFormSelect
                                ...
                                onSelect={(e) => {
                                  ...
                                  form.setValue('billDiscountRate', 0);
                                  form.setValue('billDiscountAmount', 0);
                                  form.setValue('billDiscountType', e);
                                }}
                              />
                            </Grid>
                            <Grid item xs={10} sm={10} md={10} lg={10}>

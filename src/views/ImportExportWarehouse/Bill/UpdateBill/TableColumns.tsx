...
import { NumberInput, PriceChinaInput, PriceDecimalInput, PriceInput, SaleInput } from 'plugins/NumberFormat';
...
import { SalaryInput } from 'plugins/NumberFormat';
  ...
  const columns: ProColumn<IImportExport> = useMemo(() => {
    return [
      ...
      columnHelper.accessor('discountAmount', {
        ...
        enableSorting: false,
        header: () => {
          const BDType = 'billDiscountType';
          const BDRate = 'billDiscountRate';
          const BDAmount = 'billDiscountAmount';
          const checkType = 'audienceType';
          const unitTypeD = 'unitType'
          return (
            <ProFormDependency fields={[BDType, BDRate, BDAmount,checkType,unitTypeD]}>
              {(values) => {
                const {
                  [BDType]: billDiscountType,
                    ...
                    <Tooltip title={'Chiết khấu'}>
                      <Grid container>
                        <Grid item xs={4} sm={4} md={4} lg={4}>
                          <ProFormSelect
                            disabled={
                              (billDiscountAmount !== '0' && billDiscountAmount) || audienceType === 4 || audienceType === 2 
                                ? true
                                : false
                            }
                            name="unitType"
        ...
        cell: (context) => {
          ...
          const SLSP = `form.${rowIndex}.quantity`;
          const GIATIEN = `form.${rowIndex}.price`;
          const ChietKhau = `form.${rowIndex}.discountRate`;
          ...
          return (
            ...
            >
              {(values) => {
                ...
                return (
                  // <EditableCell
                  // disable={(billDiscountAmount !== '0' && billDiscountAmount) || audienceType === 4 || audienceType === 2 ? true : false}
                  //   context={context}
                  //   FormTextFieldProps={{
                  //     InputProps: {
                  //       inputComponent: unitType === 0 ? SaleInput : PriceDecimalInput,

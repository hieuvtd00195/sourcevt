...
import { TestContext, lazy } from 'yup';
...
const ImportBill = () => {
  ...
  const handleSubmit = async (data: any) => {
    ...
    const products = form.map((item: TableCreateProducts) => ({
      ...
      //   discountRate: item.discountRate ? Number(item.discountRate) : 0,
    ...
    const body = {
      ...
      vatRate: vatRate ? vatRate : 0,
      vatAmount: vatAmount ? vatAmount : 0,
      vatBillCode: vatBillCode ? vatBillCode : null,
      vatBillDate: vatBillDate
        ? DateTime.Format(vatBillDate, 'YYYY-MM-DD')
        : null,
    ...
    try {
      ...
      if (isEmpty(products)) {
        ...
        if (response.payload === true) {
          ...
          setNotification({
            error: 'Lỗi khi thêm mới phiếu Nhập!',
          });
        }
      }
    } catch (error) {
  ...
  return (
    <PageWrapper title={'Thêm phiếu nhập'}>
      <PageBreadcrumbs
                  ...
                  <Grid item xs={12} sm={12} md={12} lg={12}>
                    ...
                    <ProFormSelect
                      ...
                      options={[
                        ...
                        { value: AUDIENCE_TYPE_KH, label: 'Khách hàng' },
